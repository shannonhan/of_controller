...
Openflow_datapath::Openflow_datapath(Datapath_manager& man)
    : datapath_state(HANDSHAKE), handshake_state(HELLO),dp_manager(man),
...
Disposition
Openflow_datapath::handle_disconnect()
{
    header_set = false;
...
Disposition
Openflow_datapath::handle_error_msg()
{
    /*
...
Disposition
Openflow_datapath::handle_handshake(const v1::ofp_msg* msg)
{
    //????????????
    //if (ofe.dp != *this)
    //    return CONTINUE;
    ...
    else if (type == v1::ofp_msg::OFPT_FEATURES_REPLY)
    {
        ...
        auto ofr = assert_cast<const v1::ofp_features_reply*>(msg);
       ...
       // manager.dispatch(dpje);
       	...
       	dp_manager.handle_datapath_join(shared_from_this());
       
    }
    else if (type == v1::ofp_msg::OFPT_HELLO)
    {
        const v1::ofp_hello* oh = assert_cast<const v1::ofp_hello*>(msg);
        if (oh->length() > v1::OFP_HELLO_BYTES)
        {
          //  VLOG_WARN(lg, "Extra-long hello (%u extra bytes)",oh->length() - v1::OFP_HELLO_BYTES);
        }
