...
Connection_manager::Connection_manager(const std::list<std::string>& interfaces, ba::io_service& io_service, Datapath_manager& man) : interfaces(interfaces), io_service_(io_service), dp_manager(man)
{
...
Connection_manager*
Connection_manager::get_instance(const std::list<std::string>& interfaces, ba::io_service& io_service_,Datapath_manager& man)
{

	return new Connection_manager(interfaces, io_service_, man);
...
{
    /* Bind/listen to interfaces */  // not refactoring here, but will in generally listen to only one port
                                   ...
                                   const boost::system::error_code& ec)
{
    ...
    boost::shared_ptr<Connection> connection(new Stream_connection<Async_stream>(socket));
    
    dp_manager.handle_new_connection(connection);
                           ...
                           const std::string& cafile)
{
    ...
    boost::shared_ptr<baip::tcp::acceptor> acceptor(new baip::tcp::acceptor(
                                                        io_service_, baip::tcp::endpoint(bind_address, port)));
...
{
    Listen_callback cb(boost::bind(&Connection_manager::listen, this, acceptor));
    boost::shared_ptr<tcp_socket> socket(new tcp_socket(io_service_));
...
// method used by SSL, not used for now...
...
/*
    ...
    if (type == SSL)
    {
        bassl::context ssl_context(io_, bassl::context::sslv23);
        ssl_context.set_options(bassl::context::default_workarounds
                                | bassl::context::no_sslv2);
        ssl_context.set_verify_mode(
            bassl::context::verify_peer |
